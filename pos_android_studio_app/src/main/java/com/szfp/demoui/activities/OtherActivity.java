...
import com.szfp.demoui.USBClass;
import com.szfp.demoui.keyboard.KeyBoardNumInterface;
import com.szfp.demoui.keyboard.KeyboardUtil;
import com.szfp.demoui.keyboard.MyKeyboardView;
import com.szfp.demoui.utils.DUKPK2009_CBC;
import com.szfp.demoui.utils.FileUtils;
import com.szfp.demoui.utils.QPOSUtil;
import com.szfp.demoui.utils.TRACE;
...
public class OtherActivity extends BaseActivity{
    ...
    private Button update_firmware,operateCardBtn, pollBtn, pollULbtn, veriftBtn, veriftULBtn, readBtn, writeBtn, finishBtn, finishULBtn, getULBtn, readULBtn, fastReadUL, writeULBtn, transferBtn,update_emv,pos_reset;
...
//                        blueTootchAddress = "/dev/ttyHSL1";//tongfang is s1，tianbo is s3
    ...
    private void initView() {
        ...
        pos_reset=findViewById(R.id.pos_reset);
        update_emv=findViewById(R.id.update_emv);
        update_firmware=findViewById(R.id.update_firmware);
    }
    ...
    private void initListener() {
        ...
        pos_reset.setOnClickListener(myOnClickListener);
        update_firmware.setOnClickListener(myOnClickListener);
        update_emv.setOnClickListener(myOnClickListener);
    }
...
//                pos.transferMifareData(data,20);
                ...
                Boolean f = pos.resetQPosStatus();
                if (f){
                    statusEditText.setText("reset FP9810 success");
                }
           /* Boolean f=pos.clearD20Device();
            Log.d("111111", f+"");
            if (f){
                statusEditText.setText("clean FP9810 log success");
            }*/
            /*Boolean f=pos.resetQPosStatus();
            if (f){
                statusEditText.setText("reset FP9810 log success");
            }*/
            }else if (v==update_emv){
                    pos.updateEMVConfigByXml(new String(FileUtils.readAssetsLine
                            ("emv_profile_tlv.xml",
                            OtherActivity.this)));
            }else if (v==update_firmware){
                Log.d("updatePosFirmware", "1111");
            if (data == null || data.length == 0) {
                data = FileUtils.readAssetsLine("220328 SZFP FP9810 _Mifare&Encr. data.asc",
                        OtherActivity.this);
            }

            int a = pos.updatePosFirmware(data, blueTootchAddress);
            Log.d("updatePosFirmware", a+"");
            if (a == -1) {
